...
const parseMeetingInput = (input) => {
  ...
  if (meetRegex.test(text))
    return {
      id: text.match(meetRegex)[1],
      type: "Google Meet",
      url: `https://meet.google.com/${text.match(meetRegex)[1]}`,
    };
  if (/^[a-z]{3}-[a-z]{4}-[a-z]{3}$/.test(text))
    return {
      id: text,
      type: "Google Meet",
      url: `https://meet.google.com/${text}`,
    };
  // 2. ZOOM
  const zoomRegex = /zoom\.us\/[jw]\/(\d+)/;
  if (zoomRegex.test(text))
    return { id: "zoom-" + text.match(zoomRegex)[1], type: "Zoom", url: text };
  if (/^\d{9,11}$/.test(text))
    return {
      id: "zoom-" + text,
      type: "Zoom",
      url: `https://zoom.us/j/${text}`,
    };
  // 3. TEAMS
  if (text.includes("teams.microsoft.com/l/meetup-join/")) {
    try {
      ...
      if (joinIndex !== -1 && pathParts[joinIndex + 1]) {
        ...
        const cleanId =
          "teams-" + rawId.replace(/[^a-zA-Z0-9]/g, "").substring(0, 30);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 style={{ margin: 0, color: "#183651", fontSize: "1.2rem" }}>
            Meeting Setup
          </h3>
          <button
            onClick={onClose}
          ...
          >
            <input
              ...
              style={{
                ...
                border: `2px solid ${isValid ? "#10b981" : "#ddd"}`,
                borderRadius: "6px",
                fontSize: "14px",
            ...
            >
              {isValid ? (
                ...
                inputValue && <span style={{ color: "#ef4444" }}>✕</span>
              )}
            </div>
          </div>
          {isValid && (
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <select
            ...
            onChange={(e) => setTempUserStatus(e.target.value)}
            style={{
              width: "100%",
              padding: "10px",
...
export const TranslationProvider = ({ children }) => {
  const copyStyles = (sourceDoc, targetDoc) => {
    Array.from(sourceDoc.styleSheets).forEach((styleSheet) => {
      try {
        ...
        console.error(
          "Skipped copying a stylesheet due to CORS security rules: ",
          e
        );
      }
    });
  ...
  const [meetingId, setMeetingId] = useState(
    () => localStorage.getItem("signSpeak_meetingId") || ""
  );
  const [userStatus, setUserStatus] = useState(
    () => localStorage.getItem("signSpeak_userStatus") || "DEAF"
  );
  ...
  const partialClearTimeoutRef = useRef(null);

  const [showConfigModal, setShowConfigModal] = useState(false);

  useEffect(() => {
    meetingIdRef.current = meetingId;
    if (meetingId) localStorage.setItem("signSpeak_meetingId", meetingId);
    else localStorage.removeItem("signSpeak_meetingId");
  }, [meetingId]);
  ...
  const handleBackendMessage = (message) => {
    if (
      ...
      try {
        ...
        // DEBUG: log incoming WS payload to help trace PARTIAL vs FINAL messages
        console.log("WS INCOMING:", data);
        if (
          data.meetingId &&
          meetingIdRef.current &&
          data.meetingId !== meetingIdRef.current
        )
          return;
        ...
        if (data.type === "AUDIO_COMMAND" && data.text) {
          ...
          // Cancel any pending clear (we're still receiving partials)
          if (partialClearTimeoutRef.current) {
            clearTimeout(partialClearTimeoutRef.current);
            partialClearTimeoutRef.current = null;
          }

          // Avoid re-setting identical partial words repeatedly
          setPartialWords((prev) => {
            const same =
              prev.length === data.words.length &&
              prev.every((w, i) => w === data.words[i]);
            return same ? prev : data.words;
          });
          ...
          // 2. CRITICAL FIX: Clear the "Ghost Words" immediately
          setPartialWords([]); 
        } else if (data.type === "CLEAR") {
          setPartialWords([]);
        } else if (data.text && !data.type) {
          // Fallback messages without a type -> only append if different from last
          setHistory((prev) => {
            if (prev.length > 0 && prev[prev.length - 1] === data.text)
              return prev;
            return [...prev, data.text];
  ...
  const connectToWebSocket = async (
    overrideMeetingId = null,
    overrideUserStatus = null,
    ...
    const currentId =
      overrideMeetingId ?? meetingIdRef.current ?? meetingId ?? "";
    let currentStatus = overrideUserStatus || userStatus;

    if (!currentId) {
      console.log(
        "No Meeting ID detected (Local Mode): Force Webcam On (DEAF Mode)"
      );
      currentStatus = "DEAF";
      autoStartCamera = true;
    }
    ...
    console.log(
      `[Connection Start] ID: ${currentId}, Role: ${currentStatus}, AutoStartCam: ${autoStartCamera}`
    );
        ...
        onOpen: async () => {
          if (currentStatus === "DEAF") {
            if (autoStartCamera) {
              ...
              console.log(
                "PiP Deaf Mode: Connesso al BE. In attesa di Start manuale."
              );
              setIsWebcamOn(false);
              setStatus("IDLE");
            }
  ...
  const readAloud = () => {
    ...
    if (
      meetingIdRef.current &&
      wsManager.stompClient &&
      wsManager.stompClient.connected
    ) {
  ...
  const restartTranslation = () => {
    ...
    // 1. Clear Frontend Visuals Immediately
    setPartialWords([]); // Wipes the "Building: HELLO WORLD..." text

    // Optional: If you also want to remove the last finalized sentence from history
    // setHistory((prev) => prev.slice(0, -1));

    const currentId = meetingIdRef.current || "";

    // 2. Tell Backend to Wipe Memory (Using WebSocket)
    if (wsManager && wsManager.stompClient && wsManager.stompClient.connected) {
      // This sends a message to Java @MessageMapping("/clear")
      // Java will then call Python's /reset_buffer
      wsManager.sendClearBuffer(currentId);
    }
    // 3. Fallback: Call Python directly if WebSocket isn't ready
    else {
      fetch("http://localhost:8000/api/reset_buffer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id: "java_default_session" }),
      }).catch((e) => console.warn("Direct reset failed", e));
    }
  };
